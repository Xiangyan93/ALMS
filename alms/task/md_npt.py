...
def _get_n_mols(n_mol: int, eq_status: int = None, in_status: int = None) -> List[SingleMolecule]:
    mols = []
    for mol in session.query(SingleMolecule).filter(or_(SingleMolecule.active == True, SingleMolecule.testset == True)):
...
class TaskNPT(TaskAL):
    ...
    def create(self, args: MonitorArgs):
        ...
        for mol in tqdm(mols, total=mols.count()):
            mol.create_jobs(task='md_npt', T_min=args.T_range[0], T_max=args.T_range[1],
    ...
    def _analyze(self, input: Tuple[Npt, str]):
    ...
    def analyze(self, args: MonitorArgs):
        ...
        for i in tqdm(range(n_analyze), total=n_analyze):
            ...
            with Pool(args.n_jobs) as p:
                results = p.map(self._analyze, [(simulator, job_dir) for job_dir in jobs_dir_])
