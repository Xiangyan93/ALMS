...
def calc_molfeatures(mol: Chem.Mol,
...
def get_features(smiles: str, features_generator: List[str] = None):
    ...
    for fg in features_generator:
        ...
        molfeatures_ = calc_molfeatures(mol, features_generator_)
        molfeatures_ = np.where(
            np.isnan(molfeatures_), replace_token, molfeatures_)
        molfeatures[fg] = molfeatures_.tolist()
    return molfeatures
